...
# ── RANK HESABLAYİCİSİ ────────────────────────────────────────────────────────
RANKS = [
    (0,    900,  "Qığılcım I",  (150, 150, 160), "⚡"),
    (900,  1000, "Qığılcım II", (180, 180, 190), "⚡"),
    (1000, 1100, "Alov I",      (255, 140, 60),  "🔥"),
    (1100, 1200, "Alov II",     (255, 110, 40),  "🔥"),
    (1200, 1350, "Nova I",      (80,  180, 255), "💫"),
    (1350, 1500, "Nova II",     (100, 200, 255), "💫"),
    (1500, 1700, "Supernova",   (160, 90,  255), "🌌"),
    (1700, 9999, "Zenit",       (255, 200, 0),   "👑"),
]
...
def _boss_silhouette(draw, cx, cy, scale, color):
    """Sadə, tanınan bir 'canavar kəllə' silueti — xarici sənət faylı olmadan çəkilir."""
    s = scale
    draw.ellipse([cx - 0.62*s, cy - 0.55*s, cx + 0.62*s, cy + 0.45*s], fill=color)
    draw.polygon([(cx - 0.5*s, cy + 0.15*s), (cx - 0.3*s, cy + 0.7*s), (cx - 0.05*s, cy + 0.25*s)], fill=color)
    draw.polygon([(cx + 0.5*s, cy + 0.15*s), (cx + 0.3*s, cy + 0.7*s), (cx + 0.05*s, cy + 0.25*s)], fill=color)
    draw.polygon([(cx - 0.45*s, cy - 0.4*s), (cx - 0.75*s, cy - 0.95*s), (cx - 0.2*s, cy - 0.55*s)], fill=color)
    draw.polygon([(cx + 0.45*s, cy - 0.4*s), (cx + 0.75*s, cy - 0.95*s), (cx + 0.2*s, cy - 0.55*s)], fill=color)
    draw.ellipse([cx - 0.32*s, cy - 0.12*s, cx - 0.1*s, cy + 0.1*s], fill=(255, 235, 235))
    draw.ellipse([cx + 0.1*s, cy - 0.12*s, cx + 0.32*s, cy + 0.1*s], fill=(255, 235, 235))
    draw.ellipse([cx - 0.26*s, cy - 0.06*s, cx - 0.14*s, cy + 0.06*s], fill=(40, 5, 5))
    draw.ellipse([cx + 0.14*s, cy - 0.06*s, cx + 0.26*s, cy + 0.06*s], fill=(40, 5, 5))


def generate_boss_event_card(boss, leaderboard, output_path):
    """boss: {current_hp, max_hp, reward_coins, defeated}. leaderboard: [{nick, damage}, ...]."""
    ROW_H = 40
    HEADER = 88
    BOSS_ZONE = 150
    BAR_ZONE = 70
    LB_TITLE_H = 30
    FOOTER = 30
    n = max(1, len(leaderboard))
    height = HEADER + BOSS_ZONE + BAR_ZONE + LB_TITLE_H + n * ROW_H + FOOTER
    ...
    boss_color = (90, 90, 100) if boss["defeated"] else accent
    _boss_silhouette(draw, WIDTH // 2, HEADER + BOSS_ZONE // 2 + 10, BOSS_ZONE * 0.62, boss_color)
    if boss["defeated"]:
        cx, cy = WIDTH // 2, HEADER + BOSS_ZONE // 2 + 10
        for sign in (-1, 1):
            ex = cx + sign * 0.21 * BOSS_ZONE * 0.62
            ey = cy
            r = 0.09 * BOSS_ZONE * 0.62
            draw.line([(ex - r, ey - r), (ex + r, ey + r)], fill=(230, 40, 40), width=4)
            draw.line([(ex - r, ey + r), (ex + r, ey - r)], fill=(230, 40, 40), width=4)

    bar_y = HEADER + BOSS_ZONE + 10
    bar_x0, bar_x1 = 28, WIDTH - 28
    bar_h = 34
    pct = boss["current_hp"] / max(boss["max_hp"], 1)
    draw.rounded_rectangle([(bar_x0, bar_y), (bar_x1, bar_y + bar_h)], radius=bar_h // 2, fill=PANEL, outline=BORDER, width=2)
    if pct > 0:
        fill_x1 = bar_x0 + (bar_x1 - bar_x0) * pct
        bar_color = GREEN if boss["defeated"] else (RED if pct < 0.3 else (230, 150, 40) if pct < 0.6 else accent)
        draw.rounded_rectangle([(bar_x0, bar_y), (fill_x1, bar_y + bar_h)], radius=bar_h // 2, fill=bar_color)
    hp_txt = "MƏĞLUB EDİLDİ" if boss["defeated"] else f"{boss['current_hp']} / {boss['max_hp']} HP  ({round(pct*100)}%)"
    draw.text((WIDTH // 2, bar_y + bar_h // 2), hp_txt, font=_font(15, True), fill=WHITE, anchor="mm")

    reward_txt = (f"Hər töhfə verən {boss['reward_coins']} coin qazanır!" if not boss["defeated"]
                  else "Növbəti boss Bazar ertəsi görünəcək")
    draw.text((WIDTH // 2, bar_y + bar_h + 20), reward_txt, font=_font(12), fill=GRAY, anchor="mm")

    y = HEADER + BOSS_ZONE + BAR_ZONE
    draw.text((28, y + 4), "ƏN ÇOX ZƏRBƏ VURANLAR", font=_font(13, True), fill=SILVER)
    y += LB_TITLE_H
    if not leaderboard:
        draw.text((28, y + 8), "Hələ heç kim zərbə vurmayıb.", font=_font(13), fill=GRAY)
    else:
        ...
        for i, p in enumerate(leaderboard):
            ...
            draw.text((32, y + ROW_H // 2), f"#{i+1}", font=_font(14, True), fill=col, anchor="lm")
            draw.text((80, y + ROW_H // 2), p["nick"], font=_font(14, True), fill=WHITE, anchor="lm")
            dmg_txt = f"{p['damage']} zərbə"
...
def generate_map_masters_card(masters: dict, output_path):
    """masters: {map_name: [{"nick", "wins", "losses", "winrate"}, ...top3]}"""
    COLS = 2
    CARD_W = (WIDTH - 28 * 2 - 16) // COLS
    CARD_H = 176
    HEADER = 88
    FOOTER = 30
    map_names = list(masters.keys())
    rows = (len(map_names) + COLS - 1) // COLS if map_names else 1
    height = HEADER + rows * (CARD_H + 14) + FOOTER
    ...
    if not map_names:
        draw.text((28, HEADER + 20), "Hələ kifayət qədər matç oynanılmayıb.", font=_font(14), fill=GRAY)
    else:
        medal_colors = [GOLD, SILVER, (176, 120, 60)]
        for i, map_name in enumerate(map_names):
            col_i, row_i = i % COLS, i // COLS
            x = 28 + col_i * (CARD_W + 16)
            y = HEADER + row_i * (CARD_H + 14)
            draw.rounded_rectangle([(x, y), (x + CARD_W, y + CARD_H)], radius=10, fill=PANEL, outline=BORDER, width=1)
            draw.text((x + 16, y + 12), map_name, font=_font(16, True), fill=WHITE)
            draw.line([(x + 16, y + 40), (x + CARD_W - 16, y + 40)], fill=BORDER, width=1)
            ry = y + 50
            for j, p in enumerate(masters[map_name][:3]):
                col = medal_colors[j] if j < 3 else GRAY
                draw.text((x + 16, ry), f"#{j+1}", font=_font(13, True), fill=col)
                draw.text((x + 46, ry), p["nick"][:16], font=_font(13, True), fill=WHITE)
                wr_txt = f"{p['winrate']}%"
