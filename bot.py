    ...
    async def open_referral(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"🏅 10 matç: **Zenith's Academy Ambassador** banneri"
            ),
            color=discord.Color.purple()
        )
        await interaction.followup.send(
...
async def sezon_elan_cmd(interaction: discord.Interaction, nomre: int, kanal: discord.TextChannel = None):
    ...
    import datetime as _dt
    embed.set_footer(text=f"Zenith's Academy  ·  {_dt.datetime.utcnow().strftime('%d.%m.%Y')}")
...
async def pass_elan_cmd(interaction: discord.Interaction, kanal: discord.TextChannel = None):
    ...
    embed = discord.Embed(
        ...
        description=(
            "**Zenith's Academy**-də Season 1 Battle Pass başladı!\n\n"
    ...
    embed.set_footer(text="Zenith's Academy  •  Season 1")
...
async def pass_cmd(interaction: discord.Interaction):
    ...
    if not has_battle_pass(interaction.user.id):
        embed = discord.Embed(
            title="Zenith's Academy SEASON 1 PASS",
            description=(
                "**Pass hele alinmayib!**\n\n"
                "**Ne qazanirsiniz:**\n"
...
async def weekly_stats_task():
    ...
    embed.set_footer(text=f"Zenith's Academy  ·  {(_dt.datetime.utcnow()+_dt.timedelta(hours=4)).strftime('%d.%m.%Y %H:%M')}")
                    ...
                    if member:
                        try:
                            await member.send(
                                ...
                                f"**Zenith's Academy Ambassador** banneri inventarına əlavə edildi!")
                        except discord.Forbidden: pass
...
async def anti_afk_check():
    ...
    for p in queue:
        ...
        if idle >= AFK_WARN_AFTER and did not in _afk_warned:
            ...
            try:
                await member.send(
                    f"⏰ **Zenith's Academy** — Sırada **{idle//60} dəqiqədir** gözləyirsən!\n"
...
async def setup_leaderboard(interaction: discord.Interaction):
    ...
    lb_embed = discord.Embed(
        title="🏆 Zenith's Academy — ÜMUMİ LEADERBOARD",
...
async def elan_cmd(interaction: discord.Interaction,
    ...
    # ── Əsas elan embedi ──────────────────────────────────────────────────────
    embed = discord.Embed(
        title="📢 Zenith's Academy — TAM YENİLƏMƏ",
        description=(
            "Salam **Zenith's Academy** cəmiyyəti!\n\n"
    ...
    embed.set_footer(text=f"Zenith's Academy  ·  {now_az}")
...
async def toplu_bildiris_cmd(interaction: discord.Interaction, mesaj: str):
    ...
    for p in players:
        ...
        if not member:
            continue
        try:
            await member.send(f"📢 **Zenith's Academy bildirisi:**\n{mesaj}")
            sent += 1
        except discord.Forbidden:
            fail += 1
    await interaction.followup.send(
...
class DavetView(discord.ui.View):
    ...
    async def show_prizes(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.add_field(name="🏅 10 Matç",   value="**Zenith's Academy Ambassador** banneri\n*(Marketdə satılmır)*", inline=True)
